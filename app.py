...
from audio_spectrogram import generate_spectrogram
...
class Sonific:
    ...
    def display_header(self):
        ...
        <div class="main-header">
            ...
            <p>Transform your 3D datasets into audio using machine learning and spatial audio rendering</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    def display_data_analysis(self, result):
        ...
        with col1:
            ...
            if 'downsampled' in stats and stats['downsampled']:
                st.markdown("""
                <div class="info-box">
                    <strong>Note:</strong> Data was intelligently downsampled for performance.
                </div>
                """, unsafe_allow_html=True)
        with col2:
    ...
    def display_audio_results(self):
        ...
        if os.path.exists(wav_file) and os.path.exists(midi_file):
            ...
            """, unsafe_allow_html=True)
            st.markdown("### Audio Frequency Analysis")
            try:
                spectrogram_path = generate_spectrogram(
                    wav_file,
                    title="Spectrogram of Your 3D Data Sonification",
                    cmap='viridis'
                )
                st.markdown("""
                <div class="spectrogram-container">
                    <h4 style="margin-bottom: 1rem; color: #667eea;">Frequency Visualization</h4>
                    <p style="margin-bottom: 1rem; color: #666;">
                        This spectrogram shows how your 3D terrain data maps to different frequencies over time.
                    </p>
                </div>
                """, unsafe_allow_html=True)
                st.image(spectrogram_path,
                        caption="Time vs Frequency visualization of your sonified 3D data",
                        use_container_width=True)
            except Exception as e:
                st.warning(f"Could not generate spectrogram: {str(e)}")
                st.info("Install librosa with: pip install librosa")
            st.markdown("### Listen to Your Data")
            ...
            if 'audio_time' in st.session_state:
                st.success(f"Generated in {st.session_state['audio_time']:.1f} seconds")
    ...
    def run(self):
        ...
        if uploaded_file:
            ...
            st.markdown("Upload a file in the sidebar to begin.")
